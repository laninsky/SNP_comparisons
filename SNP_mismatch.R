SNP_mismatch <- function(working_dir,input_structure_file) {
  ...
  for (i in (seq(1,(length(names(data))-4),2))) {
    ...
    for (j in seq((i+2),(length(names(data))),2)) {
      ...
      print(paste("Of remainging mismatches ",dim(single_dropout)[1]," (",round(((dim(single_dropout)[1])/((dim(mismatching_SNPs)[1])-(dim(hard_mismatch_both_hets)[1]))*100),2),"% of mismatching loci) are homozygous in one sample and heterozygous in the other",sep=""))
